<template>
  <div id="app" class="d-flex flex-column min-vh-100">
    <NavBar />

    <div class="container mt-4 flex-grow-1">
      <!-- Tabs Navigation -->
      <ul class="nav nav-tabs mb-3" role="tablist">
        <li class="nav-item" role="presentation">
          <button 
            class="nav-link" 
            :class="{ active: activeTab === 'world' }"
            @click="activeTab = 'world'"
            type="button"
          >
            <i class="bi bi-globe"></i> World Silver Price
          </button>
        </li>
        <li class="nav-item" role="presentation">
          <button 
            class="nav-link" 
            :class="{ active: activeTab === 'vietnam' }"
            @click="activeTab = 'vietnam'"
            type="button"
          >
            <i class="bi bi-flag"></i> Vietnam Silver Price
          </button>
        </li>
      </ul>

      <!-- Tab Content -->
      <div class="tab-content">
        <!-- World Silver Price Tab -->
        <div v-show="activeTab === 'world'" class="tab-pane fade" :class="{ 'show active': activeTab === 'world' }">
          <TradingViewChart :coin="'OANDA:XAGUSD'" />
          
          <!-- Price Alert Widget for Silver -->
          <PriceAlertWidget
            symbol="XAGUSD"
            assetType="silver"
          />
        </div>

        <!-- Vietnam Silver Price Tab -->
        <div v-show="activeTab === 'vietnam'" class="tab-pane fade" :class="{ 'show active': activeTab === 'vietnam' }">
          <div class="card">
            <div class="card-header bg-secondary text-white">
              <h5 class="mb-0">
                <i class="bi bi-coin"></i> Silver Price in Vietnam
                <span v-if="lastUpdated" class="float-end small">
                  Last Updated: {{ lastUpdated }}
                </span>
              </h5>
            </div>
            <div class="card-body">
              <div v-if="loading" class="text-center py-4">
                <div class="spinner-border text-secondary" role="status">
                  <span class="visually-hidden">Loading...</span>
                </div>
              </div>

              <div v-else-if="error" class="alert alert-danger">
                <i class="bi bi-exclamation-triangle"></i> {{ error }}
              </div>

              <div v-else-if="silverHtmlContent" v-html="silverHtmlContent" class="silver-content">
              </div>

              <div v-else class="alert alert-info">
                <i class="bi bi-info-circle"></i> No silver price data available.
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <AppFooter />
  </div>
</template>

<script>
import NavBar from './NavBar.vue';
import AppFooter from './AppFooter.vue';
import TradingViewChart from './TradingViewChart.vue'
import PriceAlertWidget from './PriceAlertWidget.vue';

export default {
  components: {
    NavBar,
    AppFooter,
    TradingViewChart,
    PriceAlertWidget,
  },
  props: {
  },
  data() {
    return {
      activeTab: 'world',
      silverHtmlContent: null,
      lastUpdated: null,
      loading: false,
      error: null,
    };
  },
  mounted() {
    // Fetch silver prices initially when component mounts
    this.fetchSilverPrices();
    // Refresh every 5 minutes
    this.refreshInterval = setInterval(() => {
      if (this.activeTab === 'vietnam') {
        this.fetchSilverPrices();
      }
    }, 5 * 60 * 1000);
  },
  watch: {
    activeTab(newTab) {
      // Fetch Vietnam silver prices when switching to Vietnam tab
      if (newTab === 'vietnam' && !this.silverHtmlContent) {
        this.fetchSilverPrices();
      }
    }
  },
  beforeUnmount() {
    if (this.refreshInterval) {
      clearInterval(this.refreshInterval);
    }
  },
  methods: {
    async fetchSilverPrices() {
      this.loading = true;
      this.error = null;

      try {
        const response = await fetch('/silverprice/silverpricePartial', {
          headers: {
            'Accept': 'text/html, */*; q=0.01',
            'X-Requested-With': 'XMLHttpRequest'
          }
        });
        
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }

        const htmlContent = await response.text();
        this.silverHtmlContent = htmlContent;
        this.lastUpdated = new Date().toLocaleString('vi-VN');
      } catch (err) {
        console.error('Error fetching silver prices:', err);
        this.error = 'Unable to load silver prices. Please try again later.';
      } finally {
        this.loading = false;
      }
    },
  },
};
</script>

<style scoped>
.table-light {
  background-color: #edf2f7;
  text-align: left;
}

.nav-tabs {
  border-bottom: 2px solid #6c757d;
  display: flex;
  flex-wrap: nowrap;
  overflow-x: auto;
  -webkit-overflow-scrolling: touch;
  scrollbar-width: thin;
  scrollbar-color: #6c757d #e2e3e5;
}

.nav-tabs::-webkit-scrollbar {
  height: 6px;
}

.nav-tabs::-webkit-scrollbar-track {
  background: #e2e3e5;
  border-radius: 3px;
}

.nav-tabs::-webkit-scrollbar-thumb {
  background: #6c757d;
  border-radius: 3px;
}

.nav-tabs::-webkit-scrollbar-thumb:hover {
  background: #5a6268;
}

.nav-item {
  flex: 0 0 auto;
  display: flex;
  min-width: fit-content;
}

.nav-tabs .nav-link {
  color: #495057;
  border: none;
  padding: 1.25rem 2rem;
  font-weight: 500;
  transition: all 0.3s ease;
  width: 100%;
  text-align: center;
  font-size: 1.1rem;
  display: flex;
  align-items: center;
  justify-content: center;
  border-radius: 0;
  white-space: nowrap;
  min-width: 200px;
}

.nav-tabs .nav-link:hover {
  color: #6c757d;
  background-color: #e2e3e5;
  border: none;
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(108, 117, 125, 0.2);
}

.nav-tabs .nav-link.active {
  color: #fff;
  background-color: #6c757d;
  border: none;
  font-weight: 600;
  box-shadow: 0 4px 12px rgba(108, 117, 125, 0.3);
}

.nav-tabs .nav-link i {
  margin-right: 0.75rem;
  font-size: 1.2rem;
}

.tab-content {
  padding-top: 1.5rem;
}

.card {
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
}

.card-header {
  font-weight: 600;
}

.silver-content {
  width: 100%;
  overflow-x: auto;
}

.silver-content >>> table {
  width: 100%;
  margin-bottom: 0;
}

.silver-content >>> table tbody tr:hover {
  background-color: #e2e3e5;
}

.badge {
  font-size: 0.85rem;
  padding: 0.35em 0.65em;
}

.text-success {
  font-weight: 600;
}

.text-danger {
  font-weight: 600;
}

@media (max-width: 768px) {
  .float-end {
    float: none !important;
    display: block;
    margin-top: 0.5rem;
  }

  .nav-tabs {
    flex-direction: row;
    overflow-x: auto;
  }

  .nav-item {
    flex: 0 0 auto;
  }

  .nav-tabs .nav-link {
    padding: 1rem 1.25rem;
    font-size: 0.95rem;
    min-width: 150px;
  }

  .nav-tabs .nav-link i {
    margin-right: 0.5rem;
    font-size: 1rem;
  }
}

/* Additional mobile breakpoint for smaller screens */
@media (max-width: 480px) {
  .nav-tabs .nav-link {
    padding: 0.75rem 1rem;
    font-size: 0.875rem;
    min-width: 130px;
  }
  
  .nav-tabs .nav-link i {
    margin-right: 0.25rem;
    font-size: 0.9rem;
  }
}
</style>
